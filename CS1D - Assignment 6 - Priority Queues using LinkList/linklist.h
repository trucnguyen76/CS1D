...
void LinkList<Type>:: deleteFirst(Type& deleteItem)
{
    ...
    {
    	cout << "There is nobody in the list";
//        throw Empty();
//Exception
    }
}
...
template <class Type>
Type& LinkList<Type>::retrieveFirst()
{
    if(!isEmpty())
    {
        return first->object;
...
template <class Type>
Type& LinkList<Type>::retrieveLast()
{
    if(!isEmpty())
    {
        return last->object;
...
template <class Type>
Type& LinkList<Type>::retrieve(int index)
{
    Node<Type>	*srchPnt;	//CALC - search pointer
